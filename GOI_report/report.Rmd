...
The following table reports clusters of integration sites identified by comparing sites recovered on or before day `r config$earlyVsLateCutoffDays` and those recovered afterwards. Clusters are identified using the geneRxCluster (ScanStatistics) software package (Berry et al. 2014). The cluster ranges in the first column are links to the UCSC genome browser where integrations with positive orientations are shown with blue tick marks while integrations with reverse orientations are shown with red tick marks. Tick marks are shaded my maximum estimated abundance values where darker shades indicated higher abundance values.

\vspace{1.0cm}

```{r, scanStats, echo=FALSE, message=FALSE, warning=FALSE}
z <- scanStatsDF
z$nearestGenes <- gsub('\\s*,\\s*', "\n", z$nearestGenes)
z$cluster <- paste0("\\href{", z$url, "}{", z$cluster, "}")
z$nearestGenes <- linebreak(z$nearestGenes, align = 'l')

knitr::kable(select(z, -url), "latex", longtable = T, linesep = "", booktabs=T, escape = F, ) %>%
  kable_styling(latex_options = c("repeat_header", "striped", "hold_position"), font_size = 9)
```
...
3. Berry CC, Ocwieja KE, Malani N, Bushman FD. Comparing DNA integration site clusters with scan statistics.
