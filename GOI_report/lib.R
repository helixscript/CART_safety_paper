...
volcanoPlot <- function(k){
  ...
  colors <- c('gray70', brewer.pal(9, 'Paired')) # grDevices::colorRampPalette(RColorBrewer::brewer.pal(12, "Paired"))(n_distinct(k$flag) - 1))
  ...
  k <- bind_rows(subset(k, oncoGene == 'No' &   geneLabel != 'FOXP1'),
                 subset(k, oncoGene == 'Yes' &  geneLabel != 'FOXP1'),
                 subset(k, oncoGene == 'TSG' &  geneLabel != 'FOXP1'),
                 subset(k, geneLabel == 'FOXP1'))
  
  p <- volcanoPlot <- ggplot(subset(k, abs(percentChange) >= 2), aes(volcanoPlot_x, volcanoPlot_y, label = geneLabel, fill = flag, shape = oncoGene)) + 
